...
(function($, Colors, undefined){
	...
	var $document = $(document),
		...
		_html = '<div class="colourPicker"><div class="cp-blank"></div><div class="cp-xy-slider"><div class="cp-white"></div><div class="cp-xy-cursor"></div></div><div class="cp-z-slider"><div class="cp-z-cursor"></div></div><div class="cp-alpha"><div class="cp-alpha-cursor"></div></div></div>',
		_css = '',
    zHeight = 164,
    ...
    $('.colour.active, .colourBack.active').removeClass('active');
      ...
      _$trigger.hasClass('noAlpha') ? $('.cp-alpha').hide() : $('.cp-alpha').show();
      if ( _$trigger.hasClass('disableable') ){
        $('.cp-blank').show();
        zHeight = 164;
      } else {
        $('.cp-blank').hide();
      ...
      $('.cp-z-slider').height(zHeight);
      _$trigger.addClass('active'), $this.parent('.colourBack').addClass('active');
			(_$UI || build()).css({
				// 'width': _$UI[0]._width,
	...
	function z_slider(event) {
		...
		_color.setColor({h: 360 - (z / zHeight * 360)}, 'hsv');
	}
	...
	function preRender(toggled) {
		var colors = _color.colors,
			...
			h = (1 - colors.hsv.h) * zHeight,
			s = colors.hsv.s * _$xy_slider._width,
			v = (1 - colors.hsv.v) * zHeight,
